...
public class ContaCorrente implements IConta, Serializable{
	...
	public void sacar(float valorSacado) throws SaldoInsuficienteException {
		...
		else if(this.saldo <= (valorSacado+(valorSacado*TAXA_SACAR_CONTA_CORRENTE))) 
		{
	...
	public void transferencia(IConta contaDestino, float valorTransferido) throws SaldoInsuficienteException, ValorInvalidoException, ContaDesativadaException, ErroInesperadoException{
		...
		else if (contaDestino instanceof ContaCorrente && (valorTransferido > 0 && (valorTransferido+(valorTransferido*TAXA_ADM_CONTA_POUPANCA)<= this.saldo))) 
		{
