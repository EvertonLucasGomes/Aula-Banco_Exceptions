...
public class ContaInvestimento implements IConta, Serializable{
	...
	public void sacar(float valorSacado) throws SaldoInsuficienteException {
		if(valorSacado > 0 && this.status && 
				this.saldo >= (valorSacado+(valorSacado*TAXA_SACAR_CONTA_INVESTIOMENTO))) 
		{
			this.saldo -= (valorSacado+(valorSacado*TAXA_SACAR_CONTA_INVESTIOMENTO));
		}
		else if(this.saldo <= (valorSacado+(valorSacado*TAXA_SACAR_CONTA_INVESTIOMENTO))) 
		{
	...
	public void transferencia(IConta contaDestino, float valorTransferido) throws SaldoInsuficienteException, ValorInvalidoException, ContaDesativadaException, ErroInesperadoException{
		if(contaDestino instanceof ContaInvestimento && (valorTransferido > 0 && (valorTransferido+(valorTransferido*TAXA_ADM_CONTA_POUPANCA)<= this.saldo)))
		{
			this.saldo -= (valorTransferido+(valorTransferido*TAXA_ADM_CONTA_POUPANCA));
		...
		{
			this.saldo -= (valorTransferido+(valorTransferido*TAXA_ADM_CONTA_CORRENTE));
			contaDestino.depositar(valorTransferido);
		}
		...
		{
			this.saldo -= (valorTransferido+(valorTransferido*TAXA_SACAR_CONTA_INVESTIOMENTO));
			contaDestino.depositar(valorTransferido);
		}
		else
